...
def _bundle_base_path() -> Path:
    ...
    if getattr(sys, "frozen", False):
        ...
        return exe_dir
    # In development, resolve to the repository root (two levels above nodebox/core/)
    current_file = Path(__file__).resolve()
    # nodebox/core/paths.py -> parents[2] is project root
    candidate = current_file.parents[2]
    if (candidate / "assets").exists() or (candidate / "qss").exists():
        return candidate
    # Fallback to parents[1] if nodebox is root
...
def resource_path(relative_path: Union[str, Path]) -> str:
    ...
    Works transparently in development and PyInstaller distributions.
    """
