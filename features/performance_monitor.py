...
class PerformanceMonitor(QWidget):
    ...
    def update_ui(self):
        ...
        self.workflows_label.setText(f"Running Workflows: {self.metrics.workflows_running}")
        self.execution_time_label.setText(f"Avg Execution Time: {self.metrics.execution_time:.3f}s")
    ...
    def update_progress_colors(self):
        ...
        if self.metrics.disk_usage > 90:
            self.disk_progress.setStyleSheet("QProgressBar::chunk { background-color: #ff4444; }")
        elif self.metrics.disk_usage > 80:
            self.disk_progress.setStyleSheet("QProgressBar::chunk { background-color: #ffaa44; }")
        else:
    ...
    def update_history_table(self):
        ...
        for i, entry in enumerate(self.history):
            self.history_table.setItem(i, 0, QTableWidgetItem(entry['timestamp'].strftime("%H:%M:%S")))
            self.history_table.setItem(i, 1, QTableWidgetItem(f"{entry['cpu']:.1f}%"))
            self.history_table.setItem(i, 2, QTableWidgetItem(f"{entry['memory']:.1f}%"))
            self.history_table.setItem(i, 3, QTableWidgetItem(f"{entry['disk']:.1f}%"))
            self.history_table.setItem(i, 4, QTableWidgetItem(str(entry['active_nodes'])))
            self.history_table.setItem(i, 5, QTableWidgetItem(str(entry['errors'])))
    
    def update_nodebox_metrics(self, active_nodes, total_nodes, workflows_running, execution_time, error_count):
